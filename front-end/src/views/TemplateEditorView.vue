  ...
  if (activeTemplate.value) {
    ...
    saveTimeout = setTimeout(async () => {
      await updateTemplateMutation({
        ...
        data: { html_content: htmlContent.value },
      })
    }, 500)
  }
})
...
async function saveTemplate() {
  ...
  await updateTemplateMutation({
    templateId: activeTemplate.value.id,
    data: { name: templateName.value, structure_id: selectedStructureId.value, html_content: htmlContent.value },
  })
...
<template>
  <div class="template-editor-view">
    ...
    <div class="toolbar mb-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
      <div class="d-flex align-items-center gap-2">
        <template v-if="activeTemplate">
          ...
          <input v-model="templateName" type="text" class="form-control form-control-sm fw-semibold" style="width: 220px" placeholder="Report name" />
          ...
          <button class="btn btn-sm btn-primary" @click="saveTemplate" title="Save template"><i class="bi bi-save"></i></button>
          <button class="btn btn-sm btn-outline-danger" @click="deleteTemplate" title="Delete template"><i class="bi bi-trash"></i></button>
        </template>
        <template v-else>
          <span class="text-muted small">Select a template from the sidebar or create one</span>
        </template>
        <button class="btn btn-sm btn-outline-primary" @click="showNewTemplateModal = true"><i class="bi bi-plus-lg me-1"></i> New</button>
      </div>
      <div class="d-flex align-items-center gap-2">
    ...
    <div class="split-view" ref="splitViewRef" :class="{ dragging: isDragging }">
      <div class="split-panel editor-panel" :style="{ width: `calc(${splitRatio}% - 6px)` }">

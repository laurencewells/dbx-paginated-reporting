...
const invalidateTemplateQueries = () => {
  ...
  if (activeProjectId.value) {
    queryClient.invalidateQueries({ queryKey: ['project-reports', activeProjectId.value] })
  }
}

const { mutateAsync: createTemplateMutation } = useCreateTemplateApiV1TemplatesPost({
  mutation: { onSuccess: invalidateTemplateQueries },
})
...
async function createTemplate() {
  ...
  try {
    const created = await createTemplateMutation({ data: { name: newTemplateName.value.trim(), structure_id: newTemplateStructureId.value, template_type: newTemplateType.value, page_size: 'email' } })
      ...
      <div v-if="activeTemplate" class="d-flex align-items-center gap-2">
        <div class="btn-group btn-group-sm" role="group">
          ...
          <button
            type="button"
            class="btn"
            :class="currentPageSize === 'A4' ? 'btn-secondary' : 'btn-outline-secondary'"
            :disabled="isLocked"
            @click="setPageSize('A4')"
            title="PDF (Experimental) — Scheduled sends attach a PDF generated by xhtml2pdf (CSS 2.1 only). Flexbox, CSS grid, gradients and box-shadow are not rendered. Use tables and simple layouts for best results."
          >
            <i class="bi bi-file-earmark-pdf me-1"></i>PDF
            <span class="badge bg-warning text-dark ms-1" style="font-size:0.6rem;vertical-align:middle;">Exp</span>
          </button>
        </div>
        <div class="btn-group btn-group-sm" role="group">
...
.editor-panel-header { background-color: var(--databricks-dark-blue); }
.editor-panel-header .panel-title { font-size: 1rem; }
.code-readonly {
  flex: 1;
  min-height: 0;
  overflow: auto;
...
.syntax-help-header { background-color: var(--databricks-dark-blue); color: white; padding: 0.75rem 1rem; display: flex; justify-content: space-between; align-items: center; }

...
_COLUMNS = (
    "id, name, fields, tables, relationships, selected_columns, sql_query, "
...
class StructuresRepository:
    ...
    async def get_all(self) -> List[Structure]:
        result = await self._require_connector().execute_query(
            f"SELECT {_COLUMNS} FROM structures ORDER BY created_at"
        )
    ...
    async def create(self, data: StructureCreate) -> Structure:
        ...
        result = await self._require_connector().execute_query(
            "INSERT INTO structures (name, fields, tables, selected_columns) "
            "VALUES (:name, CAST(:fields AS jsonb), CAST(:tables AS jsonb), "
    ...
    def _row_to_model(row) -> Structure:
        ...
        # Column order: id, name, fields, tables, relationships,
        #               selected_columns, sql_query, created_at, updated_at
        return Structure(
            id=row[0],
            name=row[1],
            fields=_parse_json(row[2]),
            tables=_parse_json(row[3]),
            relationships=_parse_json(row[4]),
            selected_columns=_parse_json(row[5]),
            sql_query=row[6],
            created_at=row[7],
            updated_at=row[8],
        )

...
from common.config import get_sql_warehouse_path
...
class SQLAuthentication:
    ...
    def __init__(self, bearer: str = None) -> None:
        ...
        self.path = get_sql_warehouse_path()
        self.bearer = bearer
    ...
    def _get_client(self) -> Connection:
        ...
        """
        if self.bearer:
            L.info("Using bearer authentication")
            return sql.connect(
        ...
        if "DATABRICKS_TOKEN" in os.environ:
            L.info("Using token authentication")
            return sql.connect(
                server_hostname=self.server,
                http_path=self.path,
            ...
            L.info("Using machine authentication")
            return sql.connect(
                server_hostname=self.server,
                http_path=self.path,
